...
#include "resource.h"
#define work 3
// 1 just off the walls							//1 просто от стен 
// 2 from one static tskok // not quite right	//2 от одного статического тскок // несовсем правильн
// 3 rebound all + works acceptably				//3 от всех отскок // + работает приемлемо
// 4 all bounce backwards						//4 от всех отскок в обратную сторону 
...
{	
		...
		for (unsigned int i = 0; i < currentCountWindow; ++i)
		{
			for (unsigned int j = 0; j < currentCountWindow; ++j)
			{
				if (i != j) 
				{
					...
					//hit the lower left corner //c2 удар нижним левым углом 
					if (
						(nextRect.top < myrect.bottom && myrect.bottom < nextRect.bottom)
						&&
						(nextRect.right > myrect.left && myrect.left > nextRect.left)
					...
					//hit the lower right corner // с1 удар нижним правым углом
					if (
						(nextRect.top < myrect.bottom && myrect.bottom < nextRect.bottom)
						&&
						(nextRect.right > myrect.right && myrect.right > nextRect.left)
					...
					// hit the top right corner // с4 удар верхним правым углом
					if (
						(nextRect.top < myrect.top && myrect.top < nextRect.bottom)
						&&
						(nextRect.right > myrect.right && myrect.right > nextRect.left)
					...
					// hit the upper left corner // с3 удар верхним левым углом
					if (
						(nextRect.top < myrect.top && myrect.top < nextRect.bottom)
						&&
						(nextRect.right > myrect.left && myrect.left > nextRect.left)
	...
	case WM_INITDIALOG:
		// not work ++++ begin
		//case WM_KEYDOWN: // как сделать при нажатии на enter  //if (wp == VK_RETURN && !was)
		if (WM_KEYDOWN) // почему не работает+++++++++++++
		{
			if (VK_CONTROL) //&& !Timer)
			{
				Timer = true;
		...
		if (!was)
		{
			...
			was = true;
			Timer = true;
			SetTimer(hWnd, 1, time, NULL);	// установка таймера по нажатию клавиши <ENTER> not work
			
			//ShellExecute(0, TEXT("open"), TEXT("calc.exe"), 0, 0, SW_SHOW);
			//ShellExecute(0, TEXT("open"), TEXT("notepad.exe"), 0, 0, SW_SHOW);

			//ptrNotepad[i].discriptor = FindWindow(TEXT("CalcFrame"), TEXT("Калькулятор")); // имя класса. // Заголовок окна.
			//ptrNotepad[i].discriptor = FindWindow(TEXT("Notepad"), TEXT("Безымянный-Блокнот"));//TEXT("Безымянный - Блокнот") GetForegroundWindow();
